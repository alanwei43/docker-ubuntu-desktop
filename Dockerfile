...
# Install Firefox
COPY ./scripts/ubuntu/install/firefox/ $INST_SCRIPTS/firefox/
COPY ./scripts/ubuntu/install/firefox/firefox.desktop $HOME/Desktop/
...
WORKDIR /data
COPY ./ ./
RUN chmod +x /data/softwares/caddy/caddy_linux_amd64 && \
  chmod +x /data/scripts/* && \
  mv /data/softwares/caddy/caddy_linux_amd64 /usr/bin/caddy
